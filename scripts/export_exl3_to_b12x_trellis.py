...
tensors into the `b12x_trellis` version-2 in-checkpoint standard
(`docs/b12x-trellis-checkpoint-format.md`). Because the standard's payload
IS the topology-neutral whole-matrix tensor layout, conversion is
metadata-only: the exporter derives the four global metadata tensors and
the `quantization_config` block from the payload and the producer's
...
produce `vectors: "per_expert"`. Sources whose gate and up hidden-axis
input vectors differ for the same expert are not representable (the
standard carries one input-scale family) and fail closed, as do
rank-sliced payloads (normalize to whole-matrix first) and non-registry
MCG multipliers.

Verification re-reads the emitted checkpoint through the b12x reader in a
temporary view directory and byte-compares payload slices and scale
vectors against the source, across the full extent and transform-block
splits.

Usage:
    python scripts/export_exl3_to_b12x_trellis.py \
        --model-dir /path/to/checkpoint --output /path/to/out \
        --verify sample --report out/conversion-report.json
"""
...
from b12x.moe._shared.trellis_checkpoint import (  # noqa: E402
    INPUT_SCALES_TENSOR,
...
def export(args: argparse.Namespace) -> dict:
    ...
    def _elect_hidden(rows: torch.Tensor) -> tuple[str, torch.Tensor]:
        per_layer = bool(torch.all(rows == rows[:, :1, :].expand_as(rows)))
        if per_layer:
            return "per_layer", rows[:, 0, :].contiguous()
        return "per_expert", rows.contiguous()

    def _elect_intermediate(
        rows: torch.Tensor,
    ...
    (output / "quantization_config.b12x_trellis.json").write_text(
        json.dumps({"quantization_config": quant_block}, indent=2) + "\n"
    )
    patched = dict(config)
    merged = dict(patched.get("quantization_config") or {})
    merged["quant_method"] = "b12x_trellis"
    merged["b12x_trellis"] = quant_block["b12x_trellis"]
    ...
    if args.verify != "none":
        _verify(model_dir, output, source, report, args)
    return report
...
def _verify(
    ...
    """Re-read the emitted standard through the b12x reader, byte-compare."""
    ...
    with tempfile.TemporaryDirectory(prefix="b12x-trellis-verify-") as tmp:
        view = pathlib.Path(tmp)
        for shard in model_dir.glob("*.safetensors"):
            (view / shard.name).symlink_to(shard)
        (view / "b12x-trellis.safetensors").unlink(missing_ok=True)
        (view / "b12x-trellis.safetensors").symlink_to(
            output / "b12x-trellis.safetensors"
        )
        (view / "config.json").write_text((output / "config.patched.json").read_text())
        checkpoint = read_trellis_checkpoint(view)
        intermediate = checkpoint.intermediate_size
        block = args.block_size
        layers = list(checkpoint.moe_layer_indices)
        if args.verify == "sample" and len(layers) > 3:
            layers = [layers[0], layers[len(layers) // 2], layers[-1]]
        extents = [(0, intermediate)]
        if intermediate // block >= 4:
        ...
        for layer_index in layers:
            for first, count in extents:
                ...
                entries = source.whole[layer_index]
                for expert in range(checkpoint.num_experts):
                    for pi, proj in enumerate(_PROJECTIONS):
                        full = _tensor(source, entries[(expert, proj)]["trellis"])
                        expected = (
                            full[:, first // 16 : (first + count) // 16]
                            if pi < 2
                            else full[first // 16 : (first + count) // 16]
                        )
                        if not torch.equal(loaded.trellis[(expert, pi)], expected):
                            raise _fail(
                                f"verification failed: layer {layer_index} "
                                f"expert {expert} {proj} extent "
                                f"[{first}, {first + count}) does not "
                                "round-trip"
                            )
                        verified += 1
            _tensor_cached.cache_clear()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="scaled-Hadamard transform block width in channels (default 128)",
    )
    parser.add_argument(
        "--verify",
        choices=("none", "sample", "full"),
        default="sample",
        help="round-trip verification through the b12x reader: 'sample' "
        "checks first/middle/last layers, 'full' checks every layer",
    )
